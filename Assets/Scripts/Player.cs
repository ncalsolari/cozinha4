...
public class Player : MonoBehaviour, IKitchenObjectParent {
    ...
    private void HandleMovement() {
        ...
        if (!canMove) {
            ...
            canMove = (moveDir.x < -.5f || moveDir.x > +.5f) && !Physics.CapsuleCast(transform.position, transform.position + (UnityEngine.Vector3.up * playerHeight), playerRadius, moveDirX, moveDistance);
            ...
            if (canMove) {
                ...
                canMove = (moveDir.z < -.5f || moveDir.z > +.5f) && !Physics.CapsuleCast(transform.position, transform.position + (UnityEngine.Vector3.up * playerHeight), playerRadius, moveDirZ, moveDistance);
